...
#define DEBUG 0					 // Debug outputs

#define HEIGHT 4                                //
#define WIDTH 3                                  // Dimensions of the playing field
int SIZE = HEIGHT*WIDTH;                         //

#define PLACEHOLDER '.'                          // Character that fills empty fields (for pritnf)
#define COLOR_COUNT 3                            // Number of different types of "gems" (7 is deafult number in Bejeweled)
...
void history_initialize()
{
 ...
 HISTORY_LATEST->board[HEIGHT-1][0] = 'A';
 HISTORY_LATEST->board[HEIGHT-1][1] = 'A';
 HISTORY_LATEST->board[HEIGHT-1][2] = 'A';
...
{
 // Get all "gems" from last 2 triplets next to each other
 if( (HISTORY_LATEST->x1 - HISTORY_LATEST->history_older->x1) + (HISTORY_LATEST->y1 - HISTORY_LATEST->history_older->y1) == 1 )
 if( (HISTORY_LATEST->x1 - HISTORY_LATEST->history_older->x2) + (HISTORY_LATEST->y1 - HISTORY_LATEST->history_older->y2) == 1 )
 if( (HISTORY_LATEST->x1 - HISTORY_LATEST->history_older->x3) + (HISTORY_LATEST->y1 - HISTORY_LATEST->history_older->y3) == 1 )

 if( (HISTORY_LATEST->x2 - HISTORY_LATEST->history_older->x1) + (HISTORY_LATEST->y2 - HISTORY_LATEST->history_older->y1) == 1 )
 if( (HISTORY_LATEST->x2 - HISTORY_LATEST->history_older->x2) + (HISTORY_LATEST->y2 - HISTORY_LATEST->history_older->y2) == 1 )
 if( (HISTORY_LATEST->x2 - HISTORY_LATEST->history_older->x3) + (HISTORY_LATEST->y2 - HISTORY_LATEST->history_older->y3) == 1 )

 if( (HISTORY_LATEST->x3 - HISTORY_LATEST->history_older->x1) + (HISTORY_LATEST->y3 - HISTORY_LATEST->history_older->y1) == 1 )
 if( (HISTORY_LATEST->x3 - HISTORY_LATEST->history_older->x2) + (HISTORY_LATEST->y3 - HISTORY_LATEST->history_older->y2) == 1 )
 if( (HISTORY_LATEST->x3 - HISTORY_LATEST->history_older->x3) + (HISTORY_LATEST->y3 - HISTORY_LATEST->history_older->y3) == 1 )

 // ISSUE: older triplets positions are not updated, if they are lifted by new ones
 ...
 printf("Nuber of triplets one move away: %d\n", check_number_of_triplets_one_move_away() );
 printf("Total ^: %d\n", check_triplets() );
 // --------------------------
...
int generate_move_check(int x1, int x2, int x3, int y1, int y2, int y3, int color)
{
 ...
 if( history_push(x1, x2, x3, y1, y2, y3, color) != 0 ) return -2;

 if( check_triplets() != 2 ){history_delete_last_entry(); return -3;}
...
// TODO: return value should be amount of game solutions found at total
int generate_move()
{
    ...
    if(HISTORY_LATEST->orientation == VERTICAL)
      {
       ...
       for(int i=0 ; i<5 ; i++) generate_move_check(x-4 + i,x-3 + i,x-2 + i,y-1,y-1,y-1,color);
       for(int i=0 ; i<4 ; i++) generate_move_check(x-5 + i,x-4 + i,x-3 + i,y,y,y,color);
       for(int i=0 ; i<5 ; i++) generate_move_check(x-4 + i,x-3 + i,x-2 + i,y+1,y+1,y+1,color);
       // Last triplet was VERTICAL and new triplet will be HORIZONTAL
       for(int i=0 ; i<3 ; i++) generate_move_check(x-3,x-3,x-3,y-2 + i,y-1 + i,y + i,color);
       for(int i=0 ; i<5 ; i++) generate_move_check(x-2,x-2,x-2,y-3 + i,y-2 + i,y-1 + i,color);
       for(int i=0 ; i<5 ; i++) generate_move_check(x-1,x-1,x-1,y-3 + i,y-2 + i,y-1 + i,color);
       for(int i=0 ; i<5 ; i++) generate_move_check(x,x,x,y-3 + i,y-2 + i,y-1 + i,color);
       for(int i=0 ; i<3 ; i++) generate_move_check(x+1,x+1,x+1,y-2 + i,y-1 + i,y + i,color);
      }
    else
      {
       // Last triplet was HORIZONTAL and new triplet will be VERTICAL
       for(int i=0 ; i<3 ; i++) generate_move_check(x-2 + i,x-1 + i,x + i,y-1,y-1,y-1,color);
       for(int i=0 ; i<3 ; i++)
         {
          generate_move_check(x-3,x-2,x-1,y+i,y+i,y+i,color);
          generate_move_check(x-2,x-1,x,y+i,y+i,y+i,color);
         }
       for(int i=0 ; i<3 ; i++) generate_move_check(x-2 + i,x-1 + i,x + i,y+3,y+3,y+3,color);
       // Last triplet was HORIZONTAL and new triplet will be HORIZONTAL too
       for(int i=0 ; i<5 ; i++) generate_move_check(x-1,x-1,x-1,y-2 + i,y-1 + i,y + i,color);
       for(int i=0 ; i<7 ; i++) generate_move_check(x,x,x,y-3 + i,y-2 + i,y-1 + i,color);
      }
   }
